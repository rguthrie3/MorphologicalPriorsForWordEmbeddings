...
import morfessor
import numpy as np
from wordvectorsorg.wordvectorsorg_interface import WordVectorsOrgInterface
...
parser.add_argument("--in-vocab-only", action="store_true", help="Only output an embedding if it is in-vocab")
parser.add_argument("--morpho-only", action="store_true", help="Only use the morpheme embeddings")
parser.add_argument("--sum-embed", action="store_true", help="Flag to generate embeddings for SumEmbed model")
options = parser.parse_args()

# Read in model
D = cPickle.load(open(options.vectors, "r"))
word_to_ix = D["word_to_ix"]
morpho_to_ix = D["morph_to_ix"]
word_embeddings = D["wordembed"]
morpho_embeddings = D["morphemeembed"]
...
with open("word_and_morpho_embeds.txt", "w") as outfile:
    ...
    if options.morpho_only:
        ...
        for word in output_words:
            ...
            WordVectorsOrgInterface.output_word_vector(word, embed, outfile)
